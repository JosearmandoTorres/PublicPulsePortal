...
type Block = {
  ...
  responses: { RespTxt: string; value?: string; RespPct?: string }[];
};
...
export default function DatabasePage() {
  ...
  useEffect(() => {
    ...
    async function loadSelections() {
      try {
        // try /selections/all fast-path
        const allRes = await fetch(`${API}/selections/all?user_id=${encodeURIComponent(DEV_USER)}`, { cache: 'no-store' });
        let items: Array<{ dataset_id: string; question_id: string }> = [];
        if (allRes.ok) {
          const j = await allRes.json();
          items = Array.isArray(j) ? j : (Array.isArray(j?.items) ? j.items : []);
        } else {
          // fallback: enumerate datasets then GET /selections per dataset
          const dsRes = await fetch(`${API}/datasets?limit=1000&offset=0`, { cache: 'no-store' });
          if (dsRes.ok) {
            const dsJson = await dsRes.json();
            const datasets: Array<{ id: string }> = Array.isArray(dsJson) ? dsJson : (Array.isArray(dsJson?.items) ? dsJson.items : []);
            for (const ds of datasets) {
              if (!ds?.id) continue;
              const r = await fetch(`${API}/selections?user_id=${encodeURIComponent(DEV_USER)}&dataset_id=${encodeURIComponent(ds.id)}`, { cache: 'no-store' });
              if (!r.ok) continue;
              ...
              for (const it of arr) {
                ...
                items.push({ dataset_id: ds.id, question_id: it.question_id });
              }
            }
          }
        }
        ...
        for (const it of items) map[`${it.dataset_id}::${it.question_id}`] = true;
        setSelected(map);
        setWorkspaceCount(items.length);
      } catch {
        // ignore, leave toggles empty
      }
    }
    loadSelections();
    return () => { ignore = true; };
